利用物理信息知情网络（Physics Informed Neural Network，PINN），考察复杂磁场（其实是核聚变反应堆的磁场）下的α粒子运动轨迹。简单的说就是把洛伦兹力运动方程改造成损失函数的形式，给神经网络输入一个时间t，让它输出对应时刻粒子的位置信息和速度信息；通过神经网络训练减低损失函数，使得神经网络的输出自然符合运动方程。
